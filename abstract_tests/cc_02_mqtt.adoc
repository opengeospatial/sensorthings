...
test-purpose:: Validate that the MQTT interface implements the request-response API.
test-method::

. execute an MQTT read request for the path for the service document.
. Validate that a service document is returned.
. Validate that the service document contains the request-response requirement.
...
test-purpose:: Validate that the MQTT interface implements the publish-subscribe bindings described in <<mqtt_publish_subscribe>>.
test-method::
