...
echo "Genering HTML..."
asciidoctor --safe -a stylesheet=rocket-panda.css -a stylesdir=./resources/stylesheets -o $DOCUMENT_HTML $DOCUMENT_SRC
cp "${DOCUMENT_HTML}" "${DIST_DIR}/${DOCUMENT_HTML}"

echo "Genering PDF..."
asciidoctor-pdf --safe -a pdf-stylesdir=resources -a pdf-style=ogc -a pdf-fontsdir=resources/fonts -o $DOCUMENT_PDF $DOCUMENT_SRC
