...
**Response:** A JSON object with a property named `+value+` and optionally a property named `+capabilities+`. The value of the property named `+value+` SHALL be a JSON Array containing one element for each entity set of the SensorThings Service. The value of the property named `+capabilities+` SHALL be a JSON Object describing the features the server supports that can not easily be detected by querying the service.
...
If the service supports the MQTT extension, then the `+capabilities+` object SHALL contain a property named `+mqtt+` of type Object, containing a property named `+endpoints+` of type Array. The JSON Array `+endpoints+` SHALL hold a list of URL Schemes that can be used to connect to the MQTT service. The Objects MAY contain additional properties.
