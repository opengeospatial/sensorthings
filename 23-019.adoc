= OGC SensorThings API 2.0
:doctype: standard
:encoding: utf-8
:lang: en
:status: draft
:committee: technical
:draft: 2.0
:external-id: http://www.opengis.net/doc/{doc-type}/{standard}/{m.n}
:docnumber: 23-019
:received-date: 2029-03-30
:issued-date: 2029-03-30
:published-date: 2029-03-30
:fullname: Hylke van der Schaaf
:fullname_2: Steve Liang
:docsubtype: implementation
:keywords: ogcdoc, OGC document, API, OData, openapi, html, MQTT
:submitting-organizations: Fraunhofer-Gesellschaft, Germany; University of Calgary, Canada; SensorUp Inc., Canada; Keys, USA; DataCove e.U., Austria
:mn-document-class: ogc
:mn-output-extensions: xml,html,doc,pdf
:local-cache-only:
:data-uri-image:
:pdf-uri: ./document.pdf
:xml-uri: ./document.xml
:doc-uri: ./document.doc
:edition: 0.1

:identifier: http://www.opengis.net/spec/sensorthings/2.0


////
Make sure to complete each included document
////
include::sections/clause_0_front_material.adoc[]

include::sections/clause_1_scope.adoc[]

include::sections/clause_2_conformance.adoc[]

include::sections/clause_3_references.adoc[]

include::sections/clause_4_terms_and_definitions.adoc[]

include::sections/clause_5_conventions.adoc[]

include::sections/clause_6_overview.adoc[]

include::sections/clause_7a_1_data_model.adoc[]

include::sections/clause_7a_2_sensing_entities.adoc[]

include::sections/clause_7a_3_sampling_entities.adoc[]

include::sections/clause_7a_4_actuation_entities.adoc[]

include::sections/clause_7b_read.adoc[]

include::sections/clause_7c_create_update_delete.adoc[]

include::sections/clause_7d_batch_requests.adoc[]

include::sections/clause_7e_data_array.adoc[]

include::sections/clause_7f_http.adoc[]

include::sections/clause_7g_mqtt.adoc[]

include::sections/clause_8_media_types.adoc[]

////
add or remove annexes after "A" as necessary
////
include::sections/annex-a.adoc[]

include::sections/annex-n.adoc[]

////
Revision History should be the last annex before the Bibliography
Bibliography should be the last annex
////
include::sections/annex-history.adoc[]

include::sections/annex-bibliography.adoc[]
