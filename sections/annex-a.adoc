...
====
[%metadata]
identifier:: {identifier-corrigendum}/conf/core/common-control-information
target:: {identifier-corrigendum}/conf/core
...
====
[%metadata]
identifier:: {identifier-corrigendum}/conf/core/entities
inherit:: {identifier-corrigendum}/conf/core/common-control-information
target:: {identifier-corrigendum}/conf/core

target:: {identifier-corrigendum}/req/party/properties
...
target:: {identifier-corrigendum}/req/relation/relations
...
====
[%metadata]
identifier:: {identifier-corrigendum}/conf/core/read
inherit:: {identifier-corrigendum}/conf/core/entities
target:: {identifier-corrigendum}/conf/core
...
test-method-type:: Manually Inspect
test-method:: Evaluate that the implementation accepts a Sensor del:[] Things API compliant HTTP del:[Get] add:[GET] request to the STAplus entities:
...
====
[%metadata]
identifier:: {identifier-corrigendum}/conf/core/default-crs/crs-definition
target:: {identifier-corrigendum}/conf/core
...
====
[%metadata]
identifier:: {identifier-corrigendum}/conf/core/default-crs/axis-order
target:: {identifier-corrigendum}/conf/core
...
====
[%metadata]
identifier:: {identifier-corrigendum}/conf/core/default-crs/media-type
target:: {identifier-corrigendum}/conf/core
...
====
[%metadata]
identifier:: {identifier-corrigendum}/conf/core/default-crs/processing
target:: {identifier-corrigendum}/conf/core
...
part:: Construct a `Location` entity that contains a `location` property whose geometry is encoded using the default-CRS and axis-order where the `encodingType` property's value is `application/geo+json` and check that the implementation del:[is] stores the geometry data without processing.
...
part:: Send an update request with HTTP del:[PATCH] add:[PUT] and check if the service responds as defined.
====
...
part:: Submit an HTTP POST request to create a `Party` entity where the `authId` is not set.
part:: Observe the response to find the location URI for the created `Party` entity.
part:: Issue an HTTP GET request to the location URI and verify that the `Party/authId` is set with the unique identifier that represents the user.

part:: Submit an HTTP POST request to create a `Party` entity where the `authId` is set with some value.
...
part:: Construct an HTTP GET request to some `Party` entity and verify that the `authId` is contained in the response.
====
...
part:: Construct an HTTP PATCH request to that **same** `Party` entity where the `authId` is set with some value.
part:: Issue an HTTP GET request to that `Party` and verify that the value of the `authId` has not changed.

test-method:: Verify that changing the `authId` is not possible for any other user:

part:: Have the user authenticate and identify the corresponding `Party`.
part:: Construct an HTTP PATCH request to *another* existing `Party` entity where the `authId` is set with some value.
part:: Issue an HTTP GET request to that other `Party` and verify that the value of the `authId` has not changed.
====
...
part:: Have the user authenticate and identify the corresponding `Party`.
part:: Construct an HTTP DELETE request to that `Party` entity.
part:: Issue an HTTP GET request to that `Party` and verify that the entity still exists.

test-method:: Verify that deleting the `authId` is not possible for any other user:

part:: Have the user authenticate and identify the corresponding `Party`.
part:: Construct an HTTP DELETE request to *another* existing `Party`.
part:: Issue an HTTP GET request to that other `Party` and verify that the entity still exists.
====
...
====
[%metadata]
identifier:: {identifier-corrigendum}/conf/business-logic/definition
target:: {identifier-corrigendum}/conf/business-logic
...
====
[%metadata]
identifier:: {identifier-corrigendum}/conf/business-logic/location
target:: {identifier-corrigendum}/conf/business-logic
...
test-method:: On the service root URI, find the JSON object with name del:[`{identifier}/conf/business-logic`] add:[`{identifier-corrigendum}/conf/business-logic`] and follow the link provided in the `href` property. Verify that the loaded HTML page contains the description of the business logic.
====
...
del:[These Conformance Class Tests remain informative until <<FG>> is an adopted OGC Standard.]

[conformance_test]
====
[%metadata]
identifier:: {identifier-corrigendum}/conf/geometry-fg/media-type
target:: {identifier-corrigendum}/conf/geometry-fg

target:: {identifier-corrigendum}/req/geometry-fg/media-type
...
====
[%metadata]
identifier:: {identifier-corrigendum}/conf/geometry-fg/default-crs
target:: {identifier-corrigendum}/conf/geometry-fg
...
====
[%metadata]
identifier:: {identifier-corrigendum}/conf/geometry-fg/supported-crs
target:: {identifier-corrigendum}/conf/geometry-fg
...
====
[%metadata]
identifier:: {identifier-corrigendum}/conf/geometry-fg/crs-error
target:: {identifier-corrigendum}/conf/geometry-fg
...
====
[%metadata]
identifier:: {identifier-corrigendum}/conf/geometry-fg/processing
target:: {identifier-corrigendum}/conf/geometry-fg
...
part:: Construct a geometry and create a `Location` and `FeatureOfInterest` entity. Verify that the geometry data is accepted and transformed to the default-CRS before add:[being] processed and stored by the implementation.
====

[conformance_test]
====
[%metadata]
identifier:: {identifier-corrigendum}/conf/geometry-fg/out
target:: {identifier-corrigendum}/conf/geometry-fg
...
test-method:: Verify that the geometry data for a `Feature` and `Location` is using default-CRS, independent del:[from] add:[of] the geometry CRS used with the creation or updating of the entity.
====
...
test-purpose:: To verify that the geometry value, compliant del:[to] add:[with] WKT is accepted as value for the `feature` and `location` property.
...
part:: Construct a geometry and create a `Location` and `FeatureOfInterest` entity. Verify that the geometry data in the `location` and `feature` properties is accepted and transformed to the default-CRS before add:[being] processed and stored by the implementation.
====
...
test-purpose:: To verify that a geometry included in a response is encoded in the default-CRS.
test-method:: Verify that the geometry data for a `feature` and `location` properties is using default-CRS, independent del:[from] add:[of] the geometry CRS used with the creation or updating of the entity.
====
...
part:: Subscribe to multiple properties of an entity set with MQTT Subscribe. Then create a new entity of the entity set. Check if a JSON object of the subscribed properties is received.
...
=== Additional STAplus *Business Logic* Conformance Class Tests

add:[These tests belong to the Business Logic Conformance Class. They are specified after the last original abstract test of STAplus 1.0 so that existing abstract test numbers remain unchanged.]

[conformance_test]
====
[%metadata]
identifier:: {identifier-corrigendum}/conf/business-logic/gdpr-note
target:: {identifier-corrigendum}/conf/business-logic

target:: {identifier-corrigendum}/req/business-logic/gdpr-note

test-purpose:: add:[To verify that the advertised Business Logic includes the GDPR compliance note for `Location` and `HistoricalLocation` of an individual party's `Thing`.]
test-method:: add:[On the service root URI, find the JSON object with name `{identifier-corrigendum}/conf/business-logic` and follow the link provided in the `href` property. Verify that the loaded HTML page states that `Location` and `HistoricalLocation` of a `Thing` associated with a `Party` whose `role` is equal to `individual` are personal data, that public access to those entities is restricted, and that access is allowed only when `Party/authId` is identical to the authenticated user identifier (`REMOTE_USER`).]
====

[conformance_test]
====
[%metadata]
identifier:: {identifier-corrigendum}/conf/business-logic/attribution
target:: {identifier-corrigendum}/conf/business-logic

target:: {identifier-corrigendum}/req/business-logic/attribution

test-purpose:: add:[To verify that the advertised Business Logic includes the License attribution note for creator identification when `License/definition` identifies an attribution license.]
test-method:: add:[On the service root URI, find the JSON object with name `{identifier-corrigendum}/conf/business-logic` and follow the link provided in the `href` property. Verify that the loaded HTML page states that `License` entities created by the implementation are templates and are read-only, that when `License/definition` does not identify an attribution license the template is associated as-is with no party-specific instance and no `attributionText`, that when `License/definition` identifies an attribution license each use is a party-specific instance of the template with a non-empty `attributionText`, that the writable properties of that instance are `attributionText` and `description` and that the associated `Party` may update them, that creator identification is always that `attributionText`, that `Party/displayName` and `Party/authId` are not used as creator identification, that a template does not store a party-specific name, that for an aggregate `MultiDatastream` whose license requires attribution the `License/attributionText` contains the copyright naming of contributors whose source license requires attribution, and that a personal name in `attributionText` of a `Party` whose `role` is equal to `individual` is personal data for which a `License` is not a legal basis.]
====
