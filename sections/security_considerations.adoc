...
Writing security considerations for implementing the OGC SensorThings API (STA) 2.0 is challenging.
This is because the range of uses of a STA implementation cannot be always known.
The use cases for an implementation could vary from automated tools for analytical purposes that fetch data (observations) to produce statistics, reports, etc. to Web-Mapping applications running in a Web Browser that interact with a STA service based on user interactions like pan, zoom in and zoom out.
...
Given that the STA Standard provides two different interfaces -- HTTP and MQTT -- and the different use cases, this section is separated into three parts: General considerations, considerations for an HTTP interface, and considerations for an MQTT interface.

[[general_security_considerations]]
...
The STA Standard specifies a conformance class that allows limiting access to the HTTP and MQTT interface to identified actors only.
...
The STA Standard does not define how authorization can be applied.
Any implementation of the STA Standard may implement authorization.
The STA Standard defines the use of HTTP status code 403 to signal to the caller that execution of the request is forbidden.

===== Integrity and Confidentiality

The STA Standard does not define how to achieve integrity and confidentiality of the stored data.
...
The STA HTTP interface is based on ODATA with an extension to define options for geospatial data.
...
An implementation of the STA Standard may support the creation of data via the HTTP and MQTT interface.
A client implementation should parse the uploaded data for scripting content.
For example cross-site scripting attacks may get executed in any Web-Application that carelessly includes data fetched from a STA HTTP or MQTT interface into an HTML page.
...
A STA HTTP endpoint that supports Cross Origin Resource Sharing (CORS) can be accessed from Web-Applications (JavaScript implementations executing in a Web Browser).
A simple Web-Application that fetches observations from such an endpoint to display a time chart potentially generates a high load on the server(s) that host the endpoint.
...
With fast zoom in and out and panning the area of interest changes and the STA HTTP endpoint receives many requests basically for nothing, because fast zoom and pan means that the user is navigating to an area of interest.
If for each of the zoom and pan steps, the STA HTTP endpoint is hit, a high server load is produced for nothing.

Another aspect regarding client development is caching: A Web Browser supports caching of static data such as images.
However, the result from a STA endpoint is a dynamically created JSON response.
This is typically not automatically cached in the Web Browser.
To avoid useless interactions with the endpoint, a Web-Application should implement efficient caching.

A STA deployment should control, via the CORS configuration, which Web-Application to trust.


[[http_security_considerations]]
==== Considerations for the HTTP Interface

Among the typical security considerations that exist for HTTP interfaces, the STA Standard specifies requirements for the use of ODATA query options.
...
To ensure that an implementation can be implemented "streaming ready", the STA Standard only defines streaming ready response types.
...
An implementation of the STA Standard should consider streaming records from the database to the HTTP interface as they are loaded from the database instead of loading the full database response into main memory before sending it to the caller.
...
A STA service implementation may expose an MQTT interface to the world.
...
The authentication mechanisms specified in the MQTT Standard can be used in an implementation of the MQTT interface of the STA Standard.
