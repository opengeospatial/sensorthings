...
To read a STAplus entity or a collection of STAplus entities, the client SHALL send del:[a] add:[an] HTTP GET request to that entity or the collection's URL. 
...
To create a STAplus entity in a collection, the client SHALL send del:[a] add:[an] HTTP POST request to that collection's URL. The POST body SHALL contain a single valid entity representation.
...
Upon successful completion, the response SHALL contain del:[a] add:[an] HTTP del:[location] add:[Location] header that contains the selfLink of the created entity.
...
A STAplus implementation that supports updates with PUT SHALL follow the requirements as defined in https://docs.ogc.org/is/18-088/18-088.html#req-create-update-delete-update-entityadd:[-put].
====
