...
As many IoT devices are resource-constrained, the SensorThings API adopts the efficient REST web service style. That means the Read, Create, Update, Delete actions can be performed on a STAplus 1.1 entity type. The requirements in this clause apply to `PartyLocation` and `PartyActivity` and follow the corresponding requirements of <<OGC22-022r2>>. If the implementation conforms to <<conformance_class_sharing_party_context>>, they also apply to `Share`.

The collection URLs use the entity-set names `PartyLocations` and `PartyActivities`. If the implementation conforms to <<conformance_class_sharing_party_context>>, the collection URL uses the entity-set name `Shares`.
...
inherit:: {identifier-101}/req-class/read
...
To read a STAplus 1.1 entity or a collection of STAplus 1.1 entities, the client SHALL send an HTTP GET request to that entity or the collection's URL as defined in <<OGC22-022r2>>.

The collection URLs SHALL use the entity-set names `PartyLocations` and `PartyActivities`. If the implementation conforms to <<conformance_class_sharing_party_context>>, the collection URL SHALL use the entity-set name `Shares`.
...
subject:: Target Type: Implementation
inherit:: {identifier}/req-class/entity-control-information
inherit:: {identifier-101}/req-class/create
...
To create a STAplus 1.1 entity in a collection, the client SHALL send an HTTP POST request to that collection's URL as defined in <<OGC22-022r2>>. The POST body SHALL contain a single valid entity representation.
...
Upon successful completion, the response SHALL contain an HTTP Location header that contains the selfLink of the created entity.
...
A request to create a STAplus 1.1 entity that includes related entities, represented using the appropriate inline representation, is referred to as a "deep insert". A STAplus implementation that supports entity creation SHALL support deep insert.
...
Upon successfully creating a STAplus 1.1 entity, the service response SHALL contain a Location header that contains the URL of the created entity. Upon successful completion the service SHALL respond with 201 Created.
====
...
subject:: Target Type: Implementation
inherit:: {identifier}/req-class/entity-control-information
inherit:: {identifier-101}/req-class/update
...
To update a STAplus 1.1 entity in a collection a STAplus implementation SHALL follow the requirements as defined in https://docs.ogc.org/is/18-088/18-088.html#req-create-update-delete-update-entity.
====
...
A STAplus implementation that supports updates with PUT SHALL follow the requirements as defined in https://docs.ogc.org/is/18-088/18-088.html#req-create-update-delete-update-entity-put.
====
...
subject:: Target Type: Implementation
inherit:: {identifier}/req-class/entity-control-information
inherit:: {identifier-101}/req-class/delete
...
To delete a STAplus 1.1 entity in a collection a STAplus implementation SHALL follow the requirements as defined in https://docs.ogc.org/is/18-088/18-088.html#req-create-update-delete-delete-entity.
====
