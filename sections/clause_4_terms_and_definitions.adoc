...
A user add:[or an autonomously acting computer, process, or similar], interacting with the service facilitating the creation of observations and managing owned resources. A party can own a thing, decide on the license of a datastream of observations, manage a campaign, be responsible del:[of] add:[for] an observation group, etc. add:[The `role` property distinguishes `individual`, `institutional`, and `system` parties.]
...
STA:: Sensor del:[] Things API
STAplus:: Sensor del:[] Things API PLUS extension
