...
The context URL consists of the URL of the service metadata document (See <<OData-CSDL>>) followed by a fragment that identifies the relevant portion of the document.
...
When a Read (GET) request is made to a URI pattern that addresses a single entity, the service returns a JSON object representing the entity, with the added properties `@context`. The `@context` annotation SHALL be a context URI as described in <<context_url>>.
...
When a Read (GET) request is made to a URI pattern that addresses a relation reference, the service responds with the  entity-id(s) of the target Entity(s).
