...
NOTE: In this corrigendum, insertions and substitutions relative to STAplus 1.0 are marked at the changed word or phrase: added text is typically rendered in red, deleted text typically struck through. Unchanged wording is not marked.
...
The STAplus extension is fully backwards compatible del:[to] add:[with] the existing OGC SensorThings API Part 1: Sensing as well as Sensing Version 1.1 and thereby offers existing deployments an easy upgrade to STAplus.

:add[This Standard is a corrigendum for STAplus 1.0. It is written as a replacement for STAplus 1.0 (OGC #22-022r1). All changes relative to STAplus 1.0 are summarized in Annex C.]
...
The STAplus extension is designed to support a model in which observations are owned by different users. This results in requirements for the ownership concept. In addition to the ownership, users may express a license for ensuring proper del:[re-use] add:[reuse] of their observations. The STAplus extension also supports expressing explicit relations between observations as well as between observations and external resources. Relations can enrich observations to enable future extensions supporting Linked Data, RDF and SPARQL. Observation group(s) allow the grouping of observations that belong together.

del:[The STAplus extension is believed to be an  important contribution towards the realization of the FAIR principles as STAplus strengthens the “I” (Interoperability) through a common data model and API as well as the “R” (Re-usability) by allowing expressing standards-based queries that may consider licensing conditions which is relevant for reuse of other users’ observations.] add:[STAplus is an important contribution to the FAIR principles. It strengthens Interoperability through a common data model and API, and Reusability by enabling standards-based queries that can take licensing conditions into account when reusing other users’ observations.] 
