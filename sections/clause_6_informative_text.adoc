...
The following sequence diagram illustrates the discovery for a URL that is not supported for subscription due to disallowed ODATA commands: `http://localhost/sta/v1.1/Datastream(4711)/Observations?$expand=FeatureOfInterest`

NOTE: It is assumed that the ODATA command `$expand` is blacklisted.
