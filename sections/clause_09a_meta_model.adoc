...
Every OData service utilizes an entity model which can be distributed over several schemas.
...
This Clause describes an overview of the types of entities in the metadata document that will be used to describe the OGC SensorThings API data model and its data model extensions.
The entities listed below are also used to describe the REST and MQTT bindings of the STA service.

The contents of this Clause are based on:
...
* `Keyed element`: Element that has a key that uniquely identifies an instance of this element amongst other instances of the same type of element.
* `Entity Type`: Named, structured, keyed element that defines the attributes and relations of Entities.
  The key for an Entity Type consists of one or more primitive attributes of the Entity Type.
...
* `Complex Type`: Named, structured, keyless, element consisting of a set of named attributes.
  ...
  An attribute with a Complex Type is called a complex attribute.
...
* `Relation`: Named connections between Entities.
  Relations are defined as `navigation attributes` in Entity Types.
...
* `Annotation`: Additional information attached to model or instance elements.
  ...
  If the annotation has an attribute name before the `@` charater, then the annotation pertains the attribute with that name.
...
This was known as the `@selfLink` in STA version 1.
|URL

|@count
|The total number of items in the requested entity set.
|Integer

|@nextLink
|In a response that returns a set of entities, this link returns the next set of entities for the main set of the response.
|URL
...
|linkName@count
|The total number of items in the related entity set of the navigation attribute `linkName`. Present when the navigation property `linkName` is an Entity Set and is expanded with the parameter `$count` set to `true`.
|Integer

|linkName@nextLink
|This link returns the next set of entities for the expanded navigation property `linkName` if that navigation property is an Entity Set.
|URL
...
<<table_type_mapping>> lists the mapping of types used in the OGC SensorThings API core data model to OData primitive types.
...
The STA core data model contains several complex types.
...
An example CSDL document describing a service hosting a STA v2.0 core data model can be found in <<sta-core-csdl-example>>.
A shortened example with comments can be found in <<csdl-example-short>>.



[[url_encoding]]
=== URL Encoding for constants
...
| Time Duration
| The keyword `duration` followed by an ISO8601 Duration in single quotes.
| `duration'P1DT30M'`

| Geometry
| The keyword `geometry` followed by Well Known Text (WKT) in single quotes.
