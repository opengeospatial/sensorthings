...
It is based on the OGC Observations, Measurements and Samples (OMS) Standard, also formalized as ISO 19156, that provides a conceptual model for capturing all relevant information pertaining to observed or measured data.
This open standard addresses the syntactic and semantic interoperability of the Internet of Things, complementing the existing IoT networking protocols such CoAP, MQTT, HTTP, 6LowPAN.
While the above-mentioned IoT networking protocols are addressing the ability for different IoT systems to exchange information, OGC SensorThings API is addressing the ability for different IoT systems to use and understand the exchanged information due to the inclusion of OMS concepts.
...
While originally conceived within the IoT world, SensorThings API has been shown to be applicable to a wide range of use cases far beyond IoT.
While SensorThings API carries Sensors in its name, its applicability ranges far beyond traditional sensor environments.
...
While excellent for providing real-time data directly from sensors, OGC SensorThings API has also successfully been deployed on legacy databases.
Thus, SensorThings API has grown from its IoT roots to become a full fledged standard for the provision of observational data, taking into account all relevant measurement metadata as foreseen under the OGC Observations, Measurements and Samples Standard.
...
* Urban Planning, Smart Cities & Digital Twins: Urban planners & municipal corporations can use the API to monitor various environmental and infrastructure metrics, like noise levels, traffic, pollution, or green space usage, helping with sustainable city planning.
...
Cities are using SensorThings API to manage their traffic systems in real-time.
SensorThings API allows for efficient, standardized data collection from these disparate systems.
...
Extensions have been created to better support user management in such scenarios.
...
While current reporting systems mostly rely on validated data provided as static datasets, approaches for direct provision of near-real-time data are increasingly being established, for which SensorThings API is uniquely suited.
...
The HTTP and MQTT interface bindings describe how the abstract API can be accessed over HTTP and/or MQTT.
...
ISO/OGC Observations, Measurements and Samples(OMS) defines conceptual models for the exchange of information describing observation acts, their results as well as the (spatial) feature on which the observation was made.
...
OMS is Topic 20 of the OGC Abstract Specification, that provides the conceptual foundation for most OGC specification development activities.
...
The data model, API, and protocol bindings are now independent sections that can be use independently.
For instance, since MQTT 5 introduced a request/response pattern, it is now possible to use the complete SensorThings API over MQTT without using the HTTP bindings to discover content.
Several extensions that were defined directly in the version 1.1 document have now been moved to a separate extension document that will be publised at a later date.
