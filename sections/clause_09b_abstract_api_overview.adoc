...
URL Patterns as defined in the OGC SensorThings API Standard consist of four parts: the Service Root, the STA API version number, the resource path and query options.
...
NOTE: The `Execute` request type is not used in the core STA API but may be used in extensions.
