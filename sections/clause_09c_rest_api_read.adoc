...
The OGC SensorThings API Standard adapts many of OData's system query options and their usage.
...
The OGC SensorThings API Standard supports a set of built-in filter operations, as described in the following table.
...
The OGC SensorThings API Standard specifies a set of functions that can be used with the $filter or $orderby query operations.
...
In order to support spatial relationship functions, SensorThings API Standard defines nine additional geospatial functions based on the spatial relationship between two geometry objects.
