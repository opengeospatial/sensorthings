...
This section describes how to read data from an OGC SensorThings API compatible service.
...
The `@context` attribute SHALL contain the Context URL (<<context_url>>): The URL of the OData context document that describes the data model and entry points of the service.

The content of the attribute named `value` SHALL be a JSON Array containing one element for each entity set of the STA Service.
Each element of the `value` array SHALL be a JSON object with at least two name/value pairs. One element has the name `name` containing the name of the entity set (e.g., Things, Locations, etc.) and one element has the name `url` containing the URL of the entity set, which may be an absolute or a relative URL.

Note: Adapted from <<ODATAJSON, section 5>>
...
The serverSettings object SHALL contain the attribute `conformance` of the type Array. It contains the URIs of all requirements classes and optional requirements that the service implements.
...
The STA Standard adapts many of OData's system query options and their usage.
These query options support refining the request.
...
Note: Adapted from <<ODATAP1, section 11.2.5.1>>
...
Note: Adapted from <<ODATAP1, section 11.2.5.2>>
...
Note: Adapted from <<ODATAP2, section 11.2.6.3>>
...
Note: Adapted from <<ODATAP1, section 11.2.6.4>>
...
Clients should be aware that the count returned in line may not exactly equal the actual number of items returned, due to latency between calculating the count and enumerating the last value or due to inexact calculations on the service.
...
Note: Adapted from <<ODATAP1, section 11.2.6.5>>
...
Note: Adapted from <<ODATAP1, section 11.2.6.2>>
...
A service SHALL support the `$orderby` query option as described in section <<read_options_orderby>>.
====
...
Note: Adapted from <<ODATAP2, section 5.1.1>>
...
Note: Adapted from <<ODATAP1, section 11.2.6.1>>
...
Entities can be filtered on attributes of related entities by using the navigationAttribute, followed by a `/`, followed by an attribute of the related entity.
...
The STA Standard supports a set of built-in filter operations, as described in the following table.
These built-in filter operator usages and definitions follow <<ODATAP1, section 11.2.6.1.1>>.
The operator precedence is described in <<ODATAP2, section 5.1.1.17>>.



[#tab-built-in-filter-operators,reftext='{table-caption} {counter:table-num}']
.Built-in Filter Operators
[cols="<20a,<25,<55a",options="header"]
|===
...
The STA Standard specifies a set of functions that can be used with the $filter or $orderby query operations.
The following table lists the available functions and they follows the OData Canonical function definitions listed in <<ODATAP2, section 5.1.1.4>>.

In order to support spatial relationship functions, the STA Standard defines nine additional geospatial functions based on the spatial relationship between two geometry objects.
The spatial relationship functions are defined in the OGC Simple Feature Access Standard <<OGC06-104, clause 6.1.2.3>>. The names of these nine functions start with a prefix `st_` following the OGC Simple Feature Access Standard.
...
|`+contains(description, 'SensorThings')+`

| `+substringof+`
|`+bool substringof(string p0, string p1)+`
|`+substringof('SensorThings',description)+`
...
|`+tolower(description) eq 'sensorthings'+`

| `+toupper+`
|`+string toupper(string p0)+`
|`+toupper(description) eq 'SENSORTHINGS'+`

| `+trim+`
|`+string trim(string p0)+`
|`+trim(description) eq 'SensorThings'+`
...
The argument of the any function is a case-sensitive, alphanumeric lambda variable name, followed by a colon (`:`), followed by a Boolean expression that can use the defined variable to access attributes of the entities in the collection.
...
This format encodes responses in JSON as specified in OData JSON Format Version 4.01.
...
The format option allows the format parameter `metadata` to control the amount of service metadata added to the response.
...
A STA clients SHALL treat the URL of the nextLink as opaque, and SHALL NOT append system query options to the URL of a next link.
Services may disallow a change of format on requests for subsequent pages using the next link.

Note: Adapted from <<ODATAP1, section 11.2.6.7>>
