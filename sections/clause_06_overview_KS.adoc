...
While originally conceived within the IoT world, SensorThings API has been shown to be applicable to a wide range of use cases far beyond IoT. While SensorThings API carries Sensors in its name, its applicability ranges far beyond traditional sensor environments. It has been successfully utilized for _ex situ_ measurements performed in the lab as well as field surveys, where humans take on the role of the Sensor. Thus, SensorThings API has grown from its IoT roots to become a full fledged standard for the provision of observational data, taking into account all relevant measurement metadata as foreseen under the OGC Observations, Measurements and Samples Standard. 
