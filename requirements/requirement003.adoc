...
part:: An implementation SHALL not return the `<Link>; rel="self"` HTTP header if the request URL either contains a denied topic or at least one denied ODATA option. In this case, the implementation SHALL return the `<Link>; rel="help"`.
